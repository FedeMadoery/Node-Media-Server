...
class NodeTransServer {
  ...
  async run() {
    try {
      ...
      Logger.error(`Node Media Trans Server startup failed. MediaRoot:${this.config.trans.mediaroot} cannot be written.`);
      return;
    }
    ...
    Logger.log(`Node Media Trans Server started for apps: [ ${apps}] , MediaRoot: ${this.config.trans.mediaroot}, ffmpeg version: ${version}`);
  }
  ...
  onPostPublish(id, streamPath, args) {
    ...
    while (i--) {
      ...
      conf.mediaroot = this.config.trans.mediaroot;
